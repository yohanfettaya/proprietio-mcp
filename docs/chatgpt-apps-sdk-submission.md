...
3. Run these prompts (read-only, deterministic against the seeded demo portfolio). In
   ...
   | *"List residents at The Madison, property prop_001, with their balance due."* | `proprietio_list_residents` with `property_id:"prop_001"` and default `include_contact_info:false` | 4 residents: Sarah Chen `$0`, Marcus Johnson `$2,350`, Elena Johnson `$0`, David Park `$4,900`. The tool response omits email, phone, internal resident IDs, unit IDs, and property IDs unless contact information is explicitly requested. |

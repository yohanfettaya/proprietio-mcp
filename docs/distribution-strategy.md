...
Status: 2026-08-25 (rev. C) · grounded in the live `proprietio-mcp` source. Supersedes
...
1. **One tool set, never renamed.** The 22 public `proprietio_*` names are the public
...
3. **V3 is tool-first, UI-ready.** Rich UI (ChatGPT widgets, Copilot Adaptive Cards) is
   still optional for listing; the V3 command-center output already includes a structured
   `ui_model` that can power a future widget/card without changing tool names.
...
├── types.ts                 # 22 public Zod INPUT schemas + debug
└── tools/
    ├── index.ts             # ToolDefinition + registry (self-checks === 23)
    ├── operations.ts (4) · properties.ts (5) · accounting.ts (6) · maintenance.ts (6) · comms.ts (1)
```
...
| UI layer | n/a | widgets (`ui://`) — future | Adaptive Cards — future |
...
Transport (stateless Streamable HTTP), the 22 public tools + Zod input schemas, the
...
- **Tool annotations** on all 22 public tools: `readOnlyHint` / `destructiveHint` / `idempotentHint` /
  `openWorldHint` + a human `title`. 18 reads, 4 writes; `update_work_order`,
...
- Verified over the wire: `tools/list` returns annotations on all 22 public tools; `tools/call`
  returns `structuredContent`. Purely additive — frozen names/schemas untouched.

**Deferred per channel:** ChatGPT `ui://` widget resources +
...
`maintenance:write` · `communications:write` → 18 public read tools, 4 write tools.
...
4. **NOW — Copilot Path A** (Copilot Studio MCP connector + Entra ID federation) in parallel
   ...
   [`copilot-submission.md`](./copilot-submission.md).
5. **DONE — V2 operations cockpit** — `proprietio_get_daily_brief` shipped as the first
   read-only prioritized operating brief across delinquency, maintenance, vacancy, and
   loss-to-lease.
6. **DONE — V3 decision layer** — `proprietio_get_command_center`,
   `proprietio_get_owner_update`, and `proprietio_get_risk_radar` add KPI tiles,
   property risk scorecards, ranked action queues, owner-ready narrative, and a future
   UI-ready `ui_model`, all read-only and no resident PII.
7. **NEXT — UI surfaces** — ChatGPT widgets and/or Copilot Adaptive Cards, only where
   conversion justifies it. **Copilot Path B** (Teams app + Graph) only if Copilot converts.
...
| ChatGPT widgets / Copilot Adaptive Cards | MED–HIGH | deferred |
